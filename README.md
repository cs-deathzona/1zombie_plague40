...
public fw_TakeDamage_Post(victim)
{
	...
	else
	{
		...
		{
			if (!get_pcvar_num(cvar_humanpainfree)) return;
		}
...
show_menu_game(id)
{
	...
	// 5. Donat
	len += formatex(menu[len], charsmax(menu) - len, "\r5.\w %L^n", id, "MENU_DONAT")

	// 6. Respawn Menu
	len += formatex(menu[len], charsmax(menu) - len, "\r6.\r %L^n^n", id, "MENU_SPECTATOR")

	// 7. Respawn Menu
	len += formatex(menu[len], charsmax(menu) - len, "\r7.\w %L^n", id, "MENU_RESPAWN")
